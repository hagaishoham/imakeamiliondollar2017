...
export default function Dashboard() {
  ...
  async function runRitual(ritual: string) {
    ...
    try {
      ...
      if (result.error) {
        setRitualMessage(`Error: ${result.error}`);
      } else if (ritual === "start") {
        setRitualMessage(result.message || "Day started!");
      } else if (ritual === "sync") {
        setRitualMessage(result.suggestions?.summary || "Sync complete!");
      } else if (ritual === "wrap-up") {
        setRitualMessage(result.summary || "Day wrapped up!");
      }
      loadDashboard();
    } catch {
      setRitualMessage("Failed to run ritual. Check your API key in Settings.");
    }
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 md:p-6 space-y-6">
      <div>
        ...
        <p className="text-muted text-sm">Your personal operating system</p>
      </div>

      {/* Rituals */}
      <div className="grid grid-cols-3 gap-3">
        {[
          { key: "start", label: "Start Day", icon: "☀", desc: "Load context" },
          { key: "sync", label: "Sync", icon: "⟳", desc: "Mid-day refresh" },
          { key: "wrap-up", label: "Wrap Up", icon: "☽", desc: "End of day" },
        ].map((r) => (
          <button
            key={r.key}
          ...
          >
            ...
            {ritualLoading === r.key && (
              <div className="text-xs text-primary mt-1 animate-pulse">Running...</div>
            )}
          </button>
        ))}
      </div>
      ...
      {nowItems.length > 0 && (
        <div className="bg-card border border-primary/30 rounded-xl p-4">
          <h2 className="font-semibold text-sm text-primary mb-2">Current Focus</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Link href="/scratchpad" className="bg-card border border-border rounded-xl p-4 hover:border-primary transition-colors">
          ...
          <div className="font-semibold">{data?.scratchpadCount || 0} items</div>
        </Link>
        <Link href="/tasks" className="bg-card border border-border rounded-xl p-4 hover:border-primary transition-colors">
          ...
          <div className="font-semibold">{waitingTasks.length} items</div>
        </Link>
        <Link href="/meetings" className="bg-card border border-border rounded-xl p-4 hover:border-primary transition-colors">
          <div className="text-2xl mb-1">◎</div>
          <div className="text-xs text-muted">Unprocessed</div>
          <div className="font-semibold">{data?.unprocessedMeetings || 0} meetings</div>
        </Link>
        <Link href="/memory" className="bg-card border border-border rounded-xl p-4 hover:border-primary transition-colors">
          ...
          <div className="font-semibold">{data?.memoryItems.length || 0} items</div>
        </Link>
      </div>
    </div>
  );
